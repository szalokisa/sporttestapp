...
export default function TestPersonsGrid(props) {
    ...
    function ShowDataChild(pr) {
        ...
        fetch(`${props.dataEndpoint}`, {
            ...
            headers: {
                ...
                select: 'ID, STT_HEAD_ID, PersonName, EmailAddress',
                top: '500',
                from: 'vTestPersons',
                where: mywhere,
                groupby: '',
    ...
    async function SaveData(saveprops) {
        ...
        axios.put(TestPersonsURL, {
            headers: {
                'Content-Type': 'application/json',
                ID: recID,
                Data: saveprops,
                HeadID: parentID,
                Identifier: 'STT_PERSONS',
                token: props.loginData.token,
            }
        })
            .then((result) => {
    ...
    const onRowValueChanged = useCallback((event) => {
        ...
        try {
            ...
            dataJson['STT_HEAD_ID'] = pID.current;
        }
        catch (err) {
            console.log('TestPersonsGrid.js (line: 185)', err);
        }
        SaveData(dataJson);
    }, []);
